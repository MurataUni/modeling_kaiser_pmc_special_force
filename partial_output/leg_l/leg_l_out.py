...
def main():
    ...
    parent_object_base = sw.load_parent_bone_inversely(pw, "thigh_l")
    name_parent = pw.fetch("thigh_l", BoneKeys.parent)
    sw.parent(parent_object_base).load_stl(os.path.join(PathInfo.dir_parts_renamed, name_parent + ".stl")).name = name_parent

    pw.remove_rotation("thigh_l")
    objects = sw.load_bones(pw, "thigh_l")
