...
def main():
    ...
    parent_object_base = sw.load_parent_bone_inversely(pw, "shoulder_adapter_r")
    name_parent = pw.fetch("shoulder_adapter_r", BoneKeys.parent)
    sw.parent(parent_object_base).load_stl(os.path.join(PathInfo.dir_parts_renamed, name_parent + ".stl")).name = name_parent

    pw.remove_rotation("shoulder_adapter_r")
    objects = sw.load_bones(pw, "shoulder_adapter_r")
